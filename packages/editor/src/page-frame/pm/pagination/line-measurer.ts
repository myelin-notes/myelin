...
export function measureLinesWithDom(
  block: PaginationBlockInfo,
  view: EditorView,
  editorScreenTop: number,
  invScale: number,
...
export class DomParagraphLineMeasurer implements ParagraphLineMeasurer {
  public measure(measurement: ParagraphLineMeasurement): ParagraphLine[] {
    return measureLinesWithDom(
      measurement.block,
      measurement.view,
      measurement.editorScreenTop,
      measurement.invScale,
      measurement.blockNaturalTop,
      measurement.blockShift,
      measurement.metrics,
      measurement.measurementCacheGeneration,
    );
  }
}
...
const domParagraphLineMeasurer = new DomParagraphLineMeasurer();
const pretextParagraphLineMeasurer = new PretextParagraphLineMeasurer();
...
export function measureParagraphLines(
  ...
  const fromDom = domParagraphLineMeasurer.measure(measurement);
  return fromDom.length > 0
    ? fromDom
    : pretextParagraphLineMeasurer.measure(measurement);
}
