...
/** Cmd/Ctrl+K quick navigation, in the app's command palette idiom. */
export function CommandPalette({ open, commands, onClose }: CommandPaletteProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-border-divider px-4 py-3">
        ...
        <ul className="max-h-80 overflow-y-auto p-2">
          {filtered.length === 0 && (
            <li className="px-3 py-6 text-center text-sm text-text-muted">
              Nothing matches. Try a region name or "download".
            </li>
          )}
          {filtered.map((command, index) => {
            ...
            return (
              <li key={command.id}>
                {showGroup && (
                  <p className="px-3 pt-2 pb-1 text-[11px] font-medium tracking-wide text-text-muted uppercase">
                    {command.group}
                  </p>
                )}
                <button
