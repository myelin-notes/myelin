      ...
      <div ref={groupRef} className="absolute left-0 top-0" onContextMenu={e => e.preventDefault()}>
        {visible && (
          <div data-wheel-container className="relative left-0 top-0 animate-in fade-in duration-150">
            <button
              className="center absolute left-0 top-0 -translate-x-1/2 -translate-y-1/2 rounded-xl size-12 bg-accent-dark shadow-ambient outline-none cursor-pointer flex items-center justify-center"
            ...
            {items.map((item, index) => {
              ...
              return (
                <button
                  key={index}
                  className="group menu-btn absolute -translate-x-1/2 -translate-y-1/2 rounded-xl size-10 backdrop-blur-xl bg-white/85 shadow-ambient outline-none cursor-pointer flex items-center justify-center transition-colors focus:bg-secondary-container"
                  style={{ left: `${positions[index][0]}px`, top: `${positions[index][1]}px` }}
                  title={item.label}
                  onClick={item.command}
                >
                  <Icon className="size-4 text-text-secondary group-focus:text-text-primary" />
                </button>
              );
            })}
          </div>
