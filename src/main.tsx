...
// Installed BEFORE the app module graph loads, with the app pulled in via dynamic import: a throw
// while *importing* a dependency then rejects the import and is captured here instead of leaving a
// blank window. The normal Logger only writes once app code calls it, so without this a pre-mount
// failure is completely silent. fatal-error has no top-level imports, so loading it cannot fail.
